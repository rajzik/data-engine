...
const check = async ({ cwd, packages, }) => {
    ...
    for (let i = 0; i < packages.length; i++) {
        ...
        projectPackages.push(await readJson(join(cwd, join('packages', project), 'package.json')));
    }
    ...
    const checkModule = (module) => {
        ...
        projectPackages.forEach((projectPackage) => {
            ...
            if (rootVersion !== projectVersion && projectVersion !== undefined) {
                invalidDependencies.push(`${module} is ${chalk.red.bold(rootVersion)} in root but ` +
...
module.exports = async params => logPromise(check(params), 'Checking runtime dependencies');
